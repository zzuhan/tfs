...
function Client(options) {
  ...
  this.appname = options.appname || 'tfscom';
...
Client.prototype.refreshServers = function () {
  ...
  urllib.request(options, function (err, data, res) {
    ...
    // 应答内容如下，第一行为客户端下次从Webservice Root Server刷新TfsAgent Server的请求间隔，客户端发送的请求个数达到此数即应该重新从Webservice Root Server获取最新列表，其下按行存放了所有TfsAgent Server的地址：
...
Client.prototype.upload = function (filepath, callback) {
  if (this.servers.length === 0) {
    // add to queue
    return this.queue.push([ filepath, callback ]);
  }
  ...
  fs.readFile(filepath, function (err, buffer) {
    ...
    urllib.request(options, args, function (err, data, res) {
      ...
      var name = data.TFS_FILE_NAME;
      if (!name) {
        // error
        err = new Error('TFS upload error, Status ' + (res && res.statusCode));
