...
function Client(options) {
  ...
  // apache服务器配置的应用路径，一般用于区分应用。如果你不需要自己独立的路径来区别，可以用默认的 "tfscom"，如果需要，请联系@微步 添加。
...
Client.prototype._request = function (cmd, callback) {
  ...
  debug('%s %s on %j', args.type, pathname, server);
  urllib.request(options, args, function (err, data, res) {
    ...
    if (res.statusCode !== 200 && res.statusCode !== 201) {
      // error
      err = new Error('TFS request error, Http status ' + (res && res.statusCode));
...
Client.prototype.upload = function (filepath, callback, timeout) {
  ...
  fs.readFile(filepath, function (err, buffer) {
    ...
    that._request(params, function (err, data) {
      if (err) {
        return callback(err);
      }
